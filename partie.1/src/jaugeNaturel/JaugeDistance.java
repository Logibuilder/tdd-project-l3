 ...
 * <ul>
 * <li>vert position courante entre les deux vigies ,
 * <li>rouge position courante supérieur à vigimax,
 * <li>bleu position courante inférieur à vigimin.
 * </ul>
 * <h6>Pour le cas des jaugesDistance, l'objet va stocker que deux valeurs: les distances entre la position courante et les deux vigis <h6>
 *
 */
public class JaugeDistance implements IJauge{
	
  private long D_position_min;
  private long D_position_max;
   ...
   * @param depart   position courante entre les xdeux vigies initiale.
   */
  public JaugeDistance(long vigieMin, long vigieMax, long depart) {
    D_position_min = depart - vigieMin;
    D_position_max = depart - vigieMax;
   ...
   *
   * @return vrai si D_position_max <= 0.
   *
   */
  public boolean estRouge() {
    return (D_position_max >= 0);
  }

  /**
   * L'état de la jauge est-il vert ?
   *
   * @return vrai si D_position_min > 0 et D_position_max > 0.
   *
   */
  public boolean estVert() {
      return (D_position_min > 0 && D_position_max < 0);
  }

  /**
   * L'état de la jauge est-il bleu ?
   *
   * @return vrai si D_position_min <= 0.
   */
  public boolean estBleu() {
    return (D_position_min <= 0);
  }

  /**
   * Incrémente le niveau d'une unité.
   * L'état peut devenir supérieur à vigieMax.
   * L'incrémentation dépend de l'état du jauge
   * Si le jauge n'est pas vert, on augmente les deux distanes de 1
   * Sinon, on fait:D_position_min += 1, D_position_max -= 1
   */
  public void incrementer() {
	  if (this.estRouge()) {
		  D_position_min += 1;
		  D_position_max += 1 ;
	  } else if (this.estVert()) {
		  D_position_min += 1;
		  D_position_max -= 1 ; 
	  } else {
		  D_position_min += 1;
   ...
   * La décrémentation dépend de l'état du jauge
   * Si le jauge n'est pas vert, on diminue les deux distanes de 1
   * Sinon, on fait:D_position_min -= 1, D_position_max += 1
   */
  ...
  public String toString() {
    return "distance entre position courante et vegieMin: " + 	Math.abs(D_position_min)+"\n\ndistance entre position courante et vegieMax: "+ Math.abs(D_position_max);
  }
}